...
| 1 | **Working voice+text loop** (P1): PWA (web + mobile) with WebRTC voice and text chat, mid-conversation switching, es/en/pt, barge-in; reachable from phone via Tailscale | Hold a natural 5-min mixed voice/text conversation in each language, from the phone | ⏳ Engineering done incl. §7 reply-language selector; dry run green. Phone field tests (2026-07-11) surfaced four defects (Hardening 7–9): resumed transcript not rendering, Spanish answered in English, a stale "Resume" button offered on an empty probe session, and no install prompt (SVG-only icons) — all root-caused, fixed, regression-tested, gates re-run green. Human acceptance run (clean, owner-only) still pending; TTFA-vs-R1 to be recorded from it |
