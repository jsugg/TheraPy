...
class _WatchdogLogger(Protocol):
...
def test_watchdog_restarts_crashed_child_until_requested_stop(
...
def test_watchdog_restarts_hung_child_and_kills_original_pid(
    free_port: FreePort, wait_until: WaitUntil
) -> None:
    port = free_port()
    child_code = (
        "import socket\n"
...
def test_watchdog_stdout_uses_only_broad_schema_and_hides_child_command(
    ...
    try:
        ...
        probe_failed_value: object = getattr(watchdog, "_probe_failed", None)
        if not callable(probe_failed_value):
            raise TypeError("watchdog probe method is unavailable")
        probe_failed = cast(Callable[[], bool], probe_failed_value)
    ...
    records = [
        require_json_object(json.loads(line), "test.watchdog.log")
...
def test_watchdog_plain_fallback_survives_broken_json(
    ...
    logger_value: object = getattr(watchdog_module, "_log", None)
    if not callable(logger_value):
        raise TypeError("watchdog logger is unavailable")
    logger = cast(_WatchdogLogger, logger_value)
    logger(
